...
<div class="px-4 sm:px-0">
    ...
    <form action="{{ route('certificates.update', $certificate) }}" method="POST" class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
        ...
        <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                <label class="flex items-center">
                    <input type="checkbox" name="is_becario" id="is_becario" value="1" {{ old('is_becario', $certificate->is_becario ?? false) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                    <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Becario</span>
                </label>
                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Puede desactivarse para convertirlo en certificado estándar. Si está activo, la fecha de expiración es obligatoria.</p>
            </div>
            ...
            <div class="sm:col-span-2">
                ...
                <div class="space-y-3">
                    @foreach($services as $service)
                        @php
                            $pivot = $certificate->services->firstWhere('id', $service->id)?->pivot;
                            $authUsername = old('service_auth_username.'.$service->id, $pivot->auth_username ?? '');
                        @endphp
                        <div class="flex flex-wrap items-center gap-2 p-2 rounded border border-gray-200 dark:border-gray-600">
                            <label class="flex items-center shrink-0">
                                <input type="checkbox" name="services[]" value="{{ $service->id }}" {{ $certificate->services->contains($service->id) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                <span class="ml-2 text-sm font-medium text-gray-700 dark:text-gray-300">{{ $service->name }}</span>
                            </label>
                            <input type="text" name="service_auth_username[{{ $service->id }}]" value="{{ $authUsername }}" placeholder="Usuario opcional (si no, se usa el email)" class="flex-1 min-w-[180px] text-sm rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
                        </div>
                    @endforeach
                </div>
            </div>
